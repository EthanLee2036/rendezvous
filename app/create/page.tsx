...
export default function CreatePage() {
  ...
  const slots = getSlots()
  if (!userId) return <div className="app"><p style={{ textAlign: 'center', padding: 80, color: 'var(--ink-muted)' }}>Loading...</p></div>

  return (
    <div className="app">
      <div className="page-header"><h1>Create Your Poll</h1><p>Set up your event, pick dates, paint time slots, and share across time zones.</p></div>
      ...
      <div className="step-footer"><a href="/" className="btn btn-secondary">← Cancel</a><button className="btn btn-primary" onClick={handleSave} disabled={saving}>{saving ? 'Creating...' : 'Create Poll →'}</button></div>
    </div>
  )
}
